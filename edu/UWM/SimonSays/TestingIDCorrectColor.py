...
class NaoShowMeGame(object):
    def __init__(ip):
        ...
        for k in self.objectDict.keys():
            self.instructionList.append((k,"Show me the "+k+" "+self.objectDict[k][2]+"."))
            ...
            while true:
                ...
                if result is None:
                    ...
                    if (cnt is not None) and (cv2.contourArea(cnt)>2000):
                        if color[1]:
                            response='That is correct! That is the '+color[0]+' '+self.objectDict[color[0]][2]+"."
                        else:
                            response='That is the '+color[0]+' '+self.objectDict[color[0]][2]+" , but Simon did not say to show me the " + color[0]+' '+self.objectDict[color[0]][2]+"."
                            correct=False
                    else:
                        seen_color=list()
                        for k in self.objectDict.keys():
